...
describe('macOS token path', () => {
  ...
  it('should correctly resolve macOS token path', () => {
    ...
    const resolvedPath = path.join(homedir, 'Library/Application Support/Granola/supabase.json');
    
    expect(resolvedPath).toBe(macPath);
    expect(resolvedPath.includes('Library/Application Support/Granola')).toBe(true);
  });
});